<script>
	...
	function setUserDetails(data) {
		...
		$('#chat_profile_image').css('background-image', `url(/wis/assets/profiles/${avatar})`);
		$('#new_message_avtar').css('background-image', `url(/wis/assets/profiles/${avatar})`);
		$('#mail_link').attr('href', `mailto:${data[0]['email']}`);

		$('#user_details_container_avtar').css('background-image', `url(/wis/assets/profiles/${avatar})`);
		...
		(data[0]['place_id']?.length > 1) ?
			$('#details_of_location').html(`<span><i class="fas fa-map-marker-alt text-dark pr-2"></i></span>${data[0]['place_id']}`) :
